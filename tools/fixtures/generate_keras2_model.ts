...
function add(class_name: string, name: string, parent: string | null, config: Record<string, unknown> = {}): void {
  layers.push({
    class_name,
    name,
...
function addMerge(class_name: string, name: string, parents: string[], config: Record<string, unknown> = {}): void {
...
const out = zipSync({
  ...
  'metadata.json': new TextEncoder().encode(JSON.stringify({ keras_version: '2.13.1', date_saved: '2026-05-27' })),
});
