...
console.log(`  external:     ${rawGraph.weights?.kind === 'external' ? 'yes' : 'no'}`);
...
// --- 2. Load checkpoint --------------------------------------------------
const indexBytes = new Uint8Array(readFileSync('test-models/vertical_tf2/variables/variables.index'));
const dataBuf = readFileSync('test-models/vertical_tf2/variables/variables.data-00000-of-00001');
const dataBuffer = dataBuf.buffer.slice(dataBuf.byteOffset, dataBuf.byteOffset + dataBuf.byteLength);
...
console.log(`\n--- checkpoint ---`);
console.log(`  checkpoint variables (excl object graph): ${variableCount}`);
...
console.log(
  `  graph.weights.totalBytes: ${graph.weights?.kind === 'available' ? graph.weights.source.totalBytes : undefined}`,
);
...
else console.log(`  orphans (first 5): ${orphans.slice(0, 5).join(', ')}${orphans.length > 5 ? '...' : ''}`);
...
for (const n of graphNodes) {
  ...
  for (const w of wis) {
    const bytes = graph.weights?.kind === 'available' ? graph.weights.source.get(w.name) : undefined;
...
for (const [initName, meta] of graph.initializers) {
  ...
  const bytes = graph.weights?.kind === 'available' ? graph.weights.source.get(initName) : undefined;
  ...
  if (!bytes) {
    if (isOrphan) noBytesOrphan++;
      ...
      mismatchConsumed++;
      if (mismatchSamples.length < 3) mismatchSamples.push(`${initName}: no bytes (expected ${expected})`);
    }
    continue;
  }
