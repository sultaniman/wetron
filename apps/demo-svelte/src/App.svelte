    ...
    >
        ...
        {#if appState.status === 'ready' && graph}
            <span style="color:{chrome.faint};font-size:13px">
                {graph.nodes.length} nodes · {graph.inputs.length} inputs · {graph.outputs.length} outputs{graph
                    .initializers.size > 0
    ...
    <main style="flex:1;position:relative;overflow:hidden">
        ...
        "
            >
                {@render brandMark(64)}
                <div
                    style="margin-top:4px;font-weight:600;font-size:22px;letter-spacing:-0.01em;color:{chrome.headline}"
                >
