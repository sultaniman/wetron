<script lang="ts">
  ...
  type WeightsLoad = { status: 'idle' } | { status: 'loading' } | { status: 'error'; message: string };
...
<div style="display:flex;flex-direction:column;height:100vh;background:{chrome.pageBg}">
  <header
    style="padding:12px 20px;background:{chrome.bg};border-bottom:1px solid {chrome.border};display:flex;align-items:center;gap:16px;flex-shrink:0"
  >
    ...
    {#if appState.status === 'ready' && graph}
      <span style="color:{chrome.faint};font-size:13px">
        {graph.nodes.length} nodes · {graph.inputs.length} inputs · {graph.outputs.length} outputs{graph.initializers
          .size > 0
          ? ` · ${graph.initializers.size} tensors`
          : ''}
      </span>
    {/if}
    ...
    {#if appState.status === 'ready' && graph?.weights?.kind === 'external' && graph.weights.format === 'savedmodel'}
      <label
        ...
        style="padding:5px 12px;background:{weightsLoad.status === 'loading'
          ? chrome.faint
          : 'transparent'};color:{weightsLoad.status === 'error'
          ? '#d93025'
          : chrome.muted};border:1px solid {weightsLoad.status === 'error'
          ? '#d93025'
          : chrome.border};border-radius:6px;cursor:{weightsLoad.status === 'loading'
          ? 'wait'
          : 'pointer'};font-size:12px;font-weight:500"
      >
        {weightsLoad.status === 'loading'
          ? 'Loading…'
    ...
    {/if}
    <label
      style="{appState.status !== 'ready'
        ? 'margin-left:auto;'
        : ''}padding:6px 14px;background:#1a73e8;color:#fff;border-radius:6px;cursor:pointer;font-size:13px;font-weight:500"
    >
      Open model
      <input
        type="file"
        accept=".onnx,.tflite,.keras,.gguf,.pb,.pte,.pt"
        style="display:none"
  ...
  <main style="flex:1;position:relative;overflow:hidden">
    {#if appState.status === 'idle'}
      ...
      >
        ...
        <div style="color:{chrome.sub};font-size:13px">Supports .onnx, .tflite, .keras, .gguf, .pt, .pte and .pb</div>
        <label
          style="margin-top:8px;padding:9px 20px;background:#1a73e8;color:#fff;border-radius:8px;cursor:pointer;font-size:14px;font-weight:500;box-shadow:0 1px 2px rgba(26,115,232,0.25)"
        >
          Open model
          <input
            type="file"
    ...
    {:else if appState.status === 'error'}
      <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:8px">
        ...
        <div style="color:{chrome.muted};font-size:13px;max-width:480px;text-align:center">
          {appState.message}
        </div>
      </div>
    {:else if appState.status === 'ready' && graph}
      ...
      >
        ...
        {#if selected !== null}
          <div
            style="position:absolute;top:16px;right:16px;width:320px;z-index:10;max-height:calc(100% - 32px);overflow-y:auto;border-radius:8px;"
          >
            <NodePropertyPanel
              target={selected}
              {graph}
