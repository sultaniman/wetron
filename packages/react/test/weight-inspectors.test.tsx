...
test("matrix defaults for rank 2, caps cells, and keeps coordinate tooltips", () => {
  render(
    <WeightPanel
      target={{ name: "w", shape: [32, 30], dtype: "float32" }}
      graph={graph([32, 30])}
    />,
  );
...
test("scalar tensors default to distribution and retain sparsity", () => {
  ...
  expect(
    Array.from((screen.getByLabelText("Weight inspector") as HTMLSelectElement).options).map(
      (option) => option.value,
    ),
...
test("rank-4 matrix exposes fixed axes and positive-only colors", () => {
  render(
    <WeightPanel
      target={{ name: "w", shape: [2, 3, 3, 3], dtype: "float32" }}
      graph={graph([2, 3, 3, 3])}
    />,
  );
...
test("tensor changes reset matrix-local slice state", () => {
  ...
  rerender(
    <WeightPanel
      target={{ name: "w2", shape: [2, 3, 3, 3], dtype: "float32" }}
      graph={graph([2, 3, 3, 3])}
    />,
  );
...
test("matrix renders positive-only and mixed-sign samples", () => {
  ...
  expect(
    new Set(screen.getAllByTestId("matrix-cell").map((cell) => cell.getAttribute("style"))).size,
  ).toBeGreaterThan(1);
});

test("distribution, axis profile, and sparsity controls update the active inspector", () => {
  render(
    <WeightPanel target={{ name: "w", shape: [2, 3], dtype: "float32" }} graph={graph([2, 3])} />,
  );
  choose("distribution");
  ...
  choose("axis");
  expect(
    screen.getByTestId("axis-profile-inspector").querySelector('[data-signed="true"]'),
  ).not.toBeNull();
...
test("long per-axis profiles virtualize distinct positions", () => {
  render(
    <WeightPanel target={{ name: "w", shape: [200], dtype: "float32" }} graph={graph([200])} />,
  );
  choose("axis");
  const profile = screen
    .getByTestId("axis-profile-inspector")
...
test("kernel gallery requires and applies an explicit layout", () => {
  render(
    <WeightPanel
      target={{ name: "w", shape: [13, 2, 3, 3], dtype: "float32" }}
      graph={graph([13, 2, 3, 3])}
    />,
  );
  choose("kernel");
...
test.each([{ shape: [4, 3, 3] }, { shape: [2, 3, 3, 3, 2] }])(
  ...
  ({ shape }) => {
    ...
    const options = Array.from(
      (screen.getByLabelText("Weight inspector") as HTMLSelectElement).options,
...
test("falls back when the selected inspector does not support the next tensor", () => {
  const { rerender } = render(
    <WeightPanel
      target={{ name: "w", shape: [2, 2, 3, 3], dtype: "float32" }}
      graph={graph([2, 2, 3, 3])}
    />,
  );
  choose("kernel");
  rerender(
    <WeightPanel target={{ name: "w2", shape: [4], dtype: "float32" }} graph={graph([4])} />,
  );
  expect(screen.getByTestId("distribution-inspector")).toBeDefined();
});
...
test("Q4_0 exposes encoded diagnostics separately", () => {
  ...
  render(
    <WeightPanel
      target={{ name: "w", shape: [32], dtype: "Q4_0" }}
      graph={graph([32], "Q4_0", bytes)}
    />,
  );
  choose("quantization");
