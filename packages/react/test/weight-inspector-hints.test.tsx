...
test("the view picker explains the active inspector", () => {
  render(
    <WeightPanel target={{ name: "w", shape: [4, 4], dtype: "float32" }} graph={graph([4, 4])} />,
  );
...
test("matrix explains its axes and reports the sampling it performed", () => {
  ...
  expect(hints().some((hint) => hint.includes("[32 × 30]") && hint.includes("[16 × 24]"))).toBe(
    true,
  );
});
...
test("axis options carry their extent", () => {
  render(
    <WeightPanel target={{ name: "w", shape: [4, 6], dtype: "float32" }} graph={graph([4, 6])} />,
  );
  const options = Array.from(
    (screen.getByLabelText("Matrix row axis") as HTMLSelectElement).options,
...
test("quantization steps through blocks instead of listing them", () => {
  ...
  render(
    <WeightPanel
      target={{ name: "w", shape: [64], dtype: "Q4_0" }}
      graph={graph([64], "Q4_0", bytes)}
    />,
  );
  choose("quantization");
...
test("diagnostics states the outlier rule and trims norm precision", () => {
  ...
  render(
    <WeightPanel
      target={{ name: "w", shape: [6, 2], dtype: "float32" }}
      graph={graph([6, 2], "float32", bytes)}
    />,
  );
  choose("diagnostics");
