  ...
  const result = useMemo(
    () =>
      inspection.status === "ready" && shape?.length
        ? computeAxisStats(inspection.values, shape, Math.min(axis, shape.length - 1))
        : null,
  ...
  const rows = visible.map((value, localIndex) => {
    ...
    return (
      <div
        ...
        key={index}
        style={
          virtual ? { position: "absolute", top: index * rowHeight, left: 0, right: 0 } : undefined
        }
      >
        <span>
          {index}
          {result.excluded[index] ? (
            <Hint text={axisExcludedHint(result.excluded[index], sliceLength)} />
          ) : null}
        </span>
        ...
        <span>
          {formatVal(
            value,
            metric === "zero-ratio" ? "float32" : (inspection.tensor.dtype ?? "float32"),
          )}
        </span>
      </div>
    );
