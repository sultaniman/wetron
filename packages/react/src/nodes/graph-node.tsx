...
export function GraphNodeComponent({ data, selected }: NodeProps<Node<GraphFlowNode['data'], 'graphNode'>>) {
  const cat = opCategory(data.opType);
  const color = `var(--wetron-category-${cat})`;
  const hasWeights = data.weightInputs.length > 0;
  const displayName = data.name && !/^op_\d+$/.test(data.name) ? data.name : undefined;
  const total = data.weightInputs.length;
