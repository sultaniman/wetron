...
test('sequential emits a sub-graph for a nested Functional layer', () => {
  const nested = {
    class_name: 'Sequential',
    config: {
      name: 'wrapped',
      layers: [
        {
          class_name: 'InputLayer',
          config: { name: 'outer_input', batch_shape: [null, 4], dtype: 'float32' },
          inbound_nodes: [],
        },
        {
          ...
          config: {
            ...
            layers: [
              {
                class_name: 'InputLayer',
                config: { name: 'inner_input', batch_shape: [null, 4], dtype: 'float32' },
                inbound_nodes: [],
              },
              {
                class_name: 'Dense',
                config: { name: 'inner_dense', units: 2, dtype: 'float32' },
                inbound_nodes: [{ args: [{ keras_history: ['inner_input', 0, 0] }], kwargs: {} }],
              },
            ],
          },
          inbound_nodes: [],
        ...
        {
          class_name: 'Dense',
          config: { name: 'head', units: 1, dtype: 'float32' },
          inbound_nodes: [],
        },
      ],
    },
  ...
  const graph = parseKeras(makeKerasZip({ 'config.json': nested }));
  expect(graph.nodes.map((node) => node.name)).toEqual(['backbone', 'head']);
  expect(graph.nodes[0].subGraph?.nodes.map((node) => node.name)).toEqual(['inner_dense']);
  expect(graph.outputs.map((output) => output.name)).toEqual(['head']);
});
...
test('MobileNetV2: 155 nodes, correct input, single output', () => {
  const bytes = new Uint8Array(readFileSync(resolve(__dirname, '../../../test-models/mobilenet.keras')));
...
for (const [label, fixture] of [
  ...
  test(`${label} fixture: structure has 3 Functional sub-graphs`, () => {
    const bytes = new Uint8Array(readFileSync(resolve(__dirname, '../../../test-models/', `${fixture}.keras`)));
...
test('Keras 2 dict-format inbound_nodes: 99 nodes, merge edges resolve', () => {
  const bytes = new Uint8Array(readFileSync(resolve(__dirname, '../../../test-models/keras2_synthetic.keras')));
  const graph = parseKeras(bytes);
