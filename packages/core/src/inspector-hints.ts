...
import { KERNEL_LAYOUTS, type KernelAxisMapping, type KernelLayoutPreset } from './weight-kernel.ts';
...
import type { WeightInspectionData } from './weight-inspection.ts';
...
const VIEW_HINTS: Readonly<Record<InspectorName, string>> = {
  ...
  distribution: 'Histogram of every decoded value. Reveals clipping, dead ranges, and quantization combs.',
...
/** Inspectors offered for a tensor, in picker order. */
export function availableInspectors(
  inspection: Pick<WeightInspectionData, 'status' | 'tensor'>,
): readonly InspectorName[] {
  if (inspection.status !== 'ready') return [];
  const shape = inspection.tensor.shape;
  const rank = shape?.length ?? 0;
  const supportsKernel = rank === 4 && shape !== null && shape.every((d) => Number.isSafeInteger(d) && d > 0);
  return [
...
/** Inspector to open a tensor with. */
export function defaultInspector(shape: readonly number[] | null): InspectorName {
  return (shape?.length ?? 0) >= 2 ? 'matrix' : 'distribution';
}

const INSPECTOR_LABELS: Partial<Record<InspectorName, string>> = {
  axis: 'per-axis profile',
  kernel: 'kernel gallery',
};

/** Display text for the inspector picker. */
export function inspectorLabel(name: InspectorName): string {
  return INSPECTOR_LABELS[name] ?? name;
}
...
export function axisExcludedHint(excluded: number, sliceLength: number): string {
  ...
  const subject = excluded === 1 ? '1 value in this slice is' : `${count(excluded)} values in this slice are`;
