...
import type { WeightInspectionData } from '../src/weight-inspection.ts';
...
function ready(shape: readonly number[] | null, dtype: string | null) {
  return {
    status: 'ready',
    tensor: { name: 'w', shape, dtype },
  } as Pick<WeightInspectionData, 'status' | 'tensor'>;
}

describe('availableInspectors', () => {
  test('returns nothing until weights are ready', () => {
    for (const status of ['deferred', 'external', 'unavailable', 'unsupported'] as const) {
      const inspection = {
        status,
        tensor: { name: 'w', shape: [4, 4], dtype: 'float32' },
      } as Pick<WeightInspectionData, 'status' | 'tensor'>;
      expect(availableInspectors(inspection)).toEqual([]);
    }
  });

  test('offers inspectors according to rank', () => {
    expect(availableInspectors(ready([4, 4], 'float32'))).toEqual([
      'matrix',
      'distribution',
      'axis',
      'sparsity',
    ...
    expect(availableInspectors(ready([8], 'float32'))).not.toContain('matrix');
    expect(availableInspectors(ready([], 'float32'))).toEqual(['distribution', 'sparsity', 'values']);
  });

  test('offers specialized inspectors only when supported', () => {
    expect(availableInspectors(ready([8, 3, 3, 3], 'float32'))).toContain('kernel');
    expect(availableInspectors(ready([8, 3, 3, 0], 'float32'))).not.toContain('kernel');
    expect(availableInspectors(ready([4, 4], 'Q4_0'))).toContain('quantization');
    expect(availableInspectors(ready([4, 4], 'float32'))).not.toContain('quantization');
  });

  test('never returns a name without a view hint', () => {
    for (const name of availableInspectors(ready([8, 3, 3, 3], 'Q4_0'))) {
      expect(inspectorViewHint(name)).toBeTruthy();
    }
  });
});

describe('defaultInspector', () => {
  test('prefers matrix at rank 2 and above', () => {
    expect(defaultInspector([4, 4])).toBe('matrix');
    expect(defaultInspector([2, 3, 4])).toBe('matrix');
  });

  test('falls back to distribution below rank 2', () => {
    expect(defaultInspector([8])).toBe('distribution');
    expect(defaultInspector([])).toBe('distribution');
    expect(defaultInspector(null)).toBe('distribution');
  });
});

describe('inspectorLabel', () => {
  test('expands abbreviated names', () => {
    expect(inspectorLabel('axis')).toBe('per-axis profile');
    expect(inspectorLabel('kernel')).toBe('kernel gallery');
    expect(inspectorLabel('matrix')).toBe('matrix');
  });
});
...
test('matrix scale hint distinguishes a sequential range from a constant one', () => {
  ...
  const flat = sampleTensorSlice(new Float64Array(16).fill(2), [4, 4], { rowAxis: 0, colAxis: 1, fixed: {} }, 8, 8);
...
test('distribution approximate hint contrasts the sample with the population', () => {
  const distribution = computeWeightDistribution(new Float64Array(100_000).map((_, index) => index / 1000));
...
test('norm outlier hint states the median-mad rule and its resolved threshold', () => {
  const shape = [6, 4];
  const values = new Float64Array(24).map((_, index) => (index >= 20 ? 50 : (Math.floor(index / 4) + 1) * 0.1));
  ...
  expect(outlier.outlier!.threshold).toBeCloseTo(outlier.outlier!.median + 6 * outlier.outlier!.deviation, 10);
