...
test('every category opCategory can return has a theme entry', () => {
  const categories = Object.keys(CATEGORY_THEME);
  expect(categories).toContain('unknown');
  expect(categories).toHaveLength(16);
  for (const opType of ['Conv', 'CONV_2D', 'Relu', 'aten::add', 'NotARealOp']) {
    expect(categories).toContain(opCategory(opType));
  }
});
