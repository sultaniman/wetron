...
test('reports non-finite coordinates, constant slices, and norm outliers', () => {
  ...
  expect(findings.some((finding) => finding.code === 'nan' && finding.coordinates[0].join(',') === '3,0')).toBe(true);
...
test('returns stable no-findings behavior and handles zero MAD', () => {
  ...
  expect(outlier.some((finding) => finding.code === 'norm-outlier' && finding.position === 3)).toBe(true);
});
