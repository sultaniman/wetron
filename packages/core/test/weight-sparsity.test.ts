...
test('computes exact and near-zero sparsity for numeric and boolean arrays', () => {
  ...
  expect(computeWeightSparsity(new Float64Array([-0, 0.01, 1, 0]), [2, 2], 0, 0.02).zeroCount).toBe(3);
...
test('returns traceable rank-4 block occupancy', () => {
  ...
  values[1] = 1;
  const blocks = computeSparsityBlocks(values, [1, 1, 4, 4], { rowAxis: 2, colAxis: 3, fixed: { 0: 0, 1: 0 } }, 2, 2);
...
test('applies near-zero thresholds to bigint weights', () => {
  const values = new BigInt64Array([0n, 1n, -1n, 2n]);
  expect(computeWeightSparsity(values, [2, 2], 0, 1).zeroCount).toBe(3);
  expect(computeSparsityBlocks(values, [2, 2], { rowAxis: 0, colAxis: 1, fixed: {} }, 2, 2, 1)[0]).toEqual(
    expect.objectContaining({ occupied: 1, empty: 3 }),
  );
});
