...
test('CATEGORY_THEME has a light and dark entry for every category', () => {
  expect(Object.keys(CATEGORY_THEME)).toHaveLength(16);
...
test('react CSS --wetron-category-* matches CATEGORY_THEME', () => {
  const css = readFileSync(resolve(__dirname, '../../react/src/model-graph-view/model-graph-view.css'), 'utf-8');
