...
async function set(element: HTMLElement, label: string, value: string) {
  const control = element.querySelector(`[aria-label="${label}"]`) as HTMLSelectElement | HTMLInputElement;
  control.value = value;
...
test('matrix explains its axes and reports the sampling it performed', async () => {
  ...
  expect(hints(element).some((hint) => hint.includes('[32 × 30]') && hint.includes('[16 × 24]'))).toBe(true);
