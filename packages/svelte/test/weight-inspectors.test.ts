...
test('scalar tensors default to distribution and retain sparsity', async () => {
  ...
  expect(
    Array.from((element.querySelector('[aria-label="Weight inspector"]') as HTMLSelectElement).options).map(
      (option) => option.value,
    ),
...
test('rank-4 matrix exposes fixed axes', async () => {
  ...
  await tick();
  expect(element.querySelector('[data-testid="matrix-cell"]')?.getAttribute('title')).toContain('[1, 0, 0, 0]');
});
...
test('matrix renders positive-only and mixed-sign samples', async () => {
  ...
  expect(
    new Set(
      Array.from(positive.querySelectorAll('[data-testid="matrix-cell"]')).map((cell) => cell.getAttribute('style')),
    ).size,
  ).toBeGreaterThan(1);
});
...
test('selector order and distribution, profile, sparsity controls match React', async () => {
  ...
  expect(labels).toEqual(['matrix', 'distribution', 'per-axis profile', 'sparsity', 'diagnostics', 'values']);
  ...
  expect(Array.from(element.querySelectorAll('[aria-label$="block"]')).every((block) => block.textContent === '')).toBe(
    true,
  );
});
...
test('kernel gallery requires OIHW selection and renders every kernel', async () => {
  ...
  expect(element.querySelector('.inspector-kernel')?.getAttribute('title')).toContain('output axis 0=0');
});
