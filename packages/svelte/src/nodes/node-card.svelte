<script lang="ts">
  ...
  const cardBg = $derived(tinted ? `color-mix(in oklch, ${color} 20%, ${tintBase})` : bg);
  const cardBorder = $derived(selected ? color : tinted ? `color-mix(in oklch, ${color} 38%, ${tintBase})` : border);
  const selectedShadow = $derived(
    selected ? `0 0 0 2px color-mix(in oklch, ${color} 25%, transparent), 0 1px 4px rgba(0,0,0,0.08)` : undefined,
  );
</script>
