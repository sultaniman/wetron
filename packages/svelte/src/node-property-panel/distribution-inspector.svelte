<script lang="ts">
    ...
    const result = $derived(ready ? computeWeightDistribution(ready.values) : null);
...
{#if ready && result && histogram}<div class="inspector" data-testid="distribution-inspector">
        ...
        <div class="inspector-stats">
            {#each percentiles as [label, value]}<span class="inspector-stat"
                    ><span class="inspector-stat-label">{label}</span><span class="inspector-stat-value">{value}</span
                    ></span
                >{/each}{#if result.approximate}<span class="inspector-stat"
                    ><span class="inspector-stat-label">approx <Hint text={distributionApproximateHint(result)} /></span
                    ><span class="inspector-stat-value">sampled</span></span
                >{/if}
        </div>
