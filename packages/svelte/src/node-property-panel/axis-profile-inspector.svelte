<script lang="ts">
    ...
    const result = $derived(
        ready && shape?.length ? computeAxisStats(ready.values, shape, Math.min(axis, shape.length - 1)) : null,
    );
    ...
    const sliceLength = $derived(
        shape?.length
            ? shape.reduce((total, dimension) => total * dimension, 1) /
                  Math.max(1, shape[Math.min(axis, shape.length - 1)])
            : 0,
    );
</script>
...
{#if ready && shape && result}<div class="inspector" data-testid="axis-profile-inspector">
        ...
        <div
            ...
            onscroll={virtual
                ? (event) =>
                      (start = Math.min(values.length - 1, Math.floor(event.currentTarget.scrollTop / rowHeight)))
                : undefined}
        >
                    ...
                    {#each visible as value, local}{@const index = start + local}{@const width = Math.max(
                        ...
                        >
                            ...
                            ><span class="inspector-profile-track" data-signed={signed || undefined}
                                ><span class="inspector-profile-bar" style="left: {left}%; width: {width}%"
                                ></span></span
                            ><span
                                >{formatVal(
                                    value,
                                    metric === 'zero-ratio' ? 'float32' : (ready.tensor.dtype ?? 'float32'),
                                )}</span
                            >
                        </div>{/each}
