                        ...
                        <span class="inspector-bounded">
                            <input
                                ...
                                oninput={(event) =>
                                    (input = Math.max(
                                        0,
                                        Math.min(shape[mapping.input] - 1, Number(event.currentTarget.value)),
                                    ))}
                            />
            ...
            {:else}
                <div class="inspector-gallery" data-testid="kernel-gallery">
                    {#each slices as slice}
                        {@const sample = sampleTensorSlice(ready.values, shape, slice.selection, 8, 8)}
                        {@const color = pickColormap(sample.min, sample.max)}
                        {@const l2 = computeKernelL2(ready.values, shape, slice.selection)}
                        <div
                        ...
                        >
                            ...
                            <div
                                class="inspector-kernel-grid"
                                style="grid-template-columns: repeat({sample.cols}, 1fr)"
                            >
                                {#each sample.cells as cell}<span
                                        title="[{cell.coordinateStart.join(', ')}]…[{cell.coordinateEnd.join(', ')}]"
                                        style="background: {colorForCell(
                                            cell.mean,
                                            sample.min,
                                            sample.max,
                                            color,
