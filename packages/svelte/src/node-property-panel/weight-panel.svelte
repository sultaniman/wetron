<script lang="ts">
    ...
    let defaultInspector = $state<WeightInspectorName>(
        untrack(() => ((target.shape?.length ?? 0) >= 2 ? 'matrix' : 'distribution')),
    );
    ...
    <div class="section">
        ...
        {#if inspection.status === 'external'}
            <div class="sizeNote">
                ...
                Load <code>variables.index</code> + <code>variables.data-00000-of-00001</code> to see stats and plots for
                this tensor.
            </div>
...
{#key target.name}
    {#if children}
        {@render children()}
    {:else}
        <DefaultWeightInspectors bind:selected={defaultInspector} />
    {/if}
{/key}
