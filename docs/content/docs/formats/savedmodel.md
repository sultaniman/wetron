...
- `VarHandleOp` nodes mark variables backed by an external checkpoint - the parser sets `ModelGraph.weights` to `{ kind: "external", format: "savedmodel" }`
...
- For TF2 models, load the checkpoint pair (`variables.index` + `variables.data-XXXXX-of-YYYYY`) with `loadSavedModelWeights` - or `loadSavedModelWeightsFromUrls(indexUrl, ...dataUrls)` for remote checkpoints - and call `attachCheckpointToGraph`. The returned graph has `weights.kind === "available"` and a source re-keyed by node name. See [Weights](../api/weights/).
